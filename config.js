...
async function onWin() {
  try {
    ...
    await callBackend(API_CONFIG.SAVE_WHEEL_RESULT, { result: 'win', ip });
    // porta alla root che legge localStorage e reindirizza correttamente
    window.location.href = '/';
  } catch (e) {
    console.error('Errore onWin:', e);
    window.location.href = '/';
  }
}

// quando il giocatore perde
async function onLose() {
  try {
    localStorage.setItem('gameResult','lose');
    const ip = await getUserIP();
    await callBackend(API_CONFIG.SAVE_WHEEL_RESULT, { result: 'lose', ip });
    window.location.href = '/';
  } catch (e) {
    console.error('Errore onLose:', e);
    window.location.href = '/';
  }
}
